    ...
    public function rows()
    {
        $query = DB::table('contract_user')
            ...
            ->select([
                ...
                'contract_user.begin as begin',
                'contract_user.expire as expire',
            ...
            ->where('contract_user.expire','>=',now())
            ->orderBy('users.name')
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($this->rows as $row)
                    <tr wire:key="contract-user-{{ $row->user_id }}-{{ $row->contract_id }}">
                        ...
                        <td class="px-4 py-3 text-gray-500">
                            {{ \Carbon\Carbon::parse($row->expire)->translatedFormat('l d F Y') }}
                        </td>
